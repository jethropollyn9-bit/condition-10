...
export default function Shop({ inventory, onQuickView, recentlyViewed }) {
  ...
  const handleAiSearch = async (e) => {
    ...
    try {
      ...
      alert("Sorry, The Vault's AI is taking a quick break. Try a standard search.");
    } finally {
        ...
        <div className="flex flex-col gap-6 w-full">
          <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            ...
            <div className="flex gap-4 w-full md:w-auto">
              <input type="text" placeholder="STANDARD SEARCH" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="bg-zinc-100 text-zinc-900 text-xs font-bold tracking-widest uppercase px-5 py-2.5 rounded-full outline-none w-full sm:w-64 focus:ring-2 focus:ring-purple-600 transition-all placeholder:text-zinc-400 border-none" />
