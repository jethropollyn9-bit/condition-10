          ...
          <div className="flex flex-col gap-3">
            <form onSubmit={handleAiSearch} className="w-full bg-purple-50 rounded-2xl p-4 border border-purple-100 flex flex-col sm:flex-row gap-3 items-center shadow-inner">
              <div className="flex-1 w-full flex items-center gap-3 bg-white px-5 py-3.5 rounded-xl shadow-sm border border-purple-50 focus-within:ring-2 focus-within:ring-purple-600 transition-all">
                <input type="text" placeholder='e.g., "Show me casual white trainers under £150"' value={aiQuery} onChange={(e) => setAiQuery(e.target.value)} className="w-full bg-transparent border-none outline-none text-sm font-medium text-zinc-900 placeholder:text-zinc-400" disabled={isAiLoading} />
              </div>
              <button type="submit" disabled={isAiLoading || !aiQuery.trim()} className={`w-full sm:w-auto px-8 py-3.5 rounded-xl font-bold tracking-widest uppercase text-xs transition-all shadow-md flex justify-center items-center gap-2 ${isAiLoading || !aiQuery.trim() ? 'bg-zinc-200 text-zinc-400 cursor-not-allowed' : 'bg-purple-600 text-white hover:bg-zinc-900 hover:shadow-lg cursor-pointer'}`}>
                {isAiLoading ? 'Analyzing...' : 'Ask AI'}
              </button>
            </form>

            {aiFeedback && (
              <div className={`w-full px-5 py-3 rounded-xl text-[10px] font-black tracking-widest uppercase transition-all duration-300 shadow-sm border ${aiFeedback.type === 'error' ? 'bg-red-50 text-red-600 border-red-200' : 'bg-purple-100 text-purple-700 border-purple-200'}`}>
          ...
          <div className="mt-16 flex flex-col items-center justify-center gap-6">
            {visibleCount < displayedSneakers.length && (
              <button 
                ...
                className="bg-white border-2 border-zinc-900 text-zinc-900 px-8 py-3 rounded-full font-bold uppercase tracking-widest text-xs hover:bg-zinc-900 hover:text-white transition-colors cursor-pointer"
              >
                Load More Vault Items
              </button>
            )}
